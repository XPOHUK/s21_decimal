...
int big_decimal_mul_big_int(big_decimal first, big_decimal second,
  ...
  if (big_decimal_get_not_zero_bit(first) >
      big_decimal_get_not_zero_bit(second)) {
    multipliable = first;
    multiplier = second;
  } else {
    multipliable = second;
  ...
  for (register int i = 0; i <= count; i++) {
    if (big_decimal_is_set_bit(multiplier, i)) {
      sum =
          big_decimal_add_big_int(sum, big_decimal_shift_left(multipliable, i));
    }
  }
