...
/**
 * @brief Функция выравнивания экспонент. Принимает на входе два указателя на
 * big_decimal экспоненты которых надо выровнять. Выравнивание производится
 * домножением мантиссы на 10 и увеличением экспоненты на 1 для числа с меньшей
 * экспонентой. Операция производится до тех пор пока экспоненты обоих чисел не
 * сравняются. Подразумевается, что корректные s21_decimal конвертируются в
 * big_decimal и подаются на вход функции. В данном случае, теоретически,
 * переполнения big_decimal при умножении произойти не должно.
 *
 * @param first
 * @param second
 */
