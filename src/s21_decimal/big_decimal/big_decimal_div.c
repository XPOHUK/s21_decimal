        ...
        if (temp_exp != 28) {
            ...
            if (big_decimal_is_zero(temp_res) || big_decimal_get_not_zero_bit(temp_res) < 96 ) {
                ...
                *result = temp_res;
                ...
                // Произошло переполнение при умножении основного результата. Отбрасываем переполненный результат, т.е. не сохраняем в result.
    ...
    //     printf("bug\n");
        ...
        if (div_res.parts[0] > 5 || (div_res.parts[0] == 5 && (!big_decimal_is_zero(remainder) || big_decimal_is_set_bit(*result, 0)))) {
            ...
            if (big_decimal_get_not_zero_bit(temp) > 95) {
                ...
                big_decimal_div_big_int(*result, decimal_to_big_decimal(s21_decimal_get_ten()), result, &temp_rem);
