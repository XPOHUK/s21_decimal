...
/**
 * @brief Функция возвращает big_decimal с одним единственным битом мантиссы
...
/**
 * @brief Функция устанваливает в указанный бит мантиссы 1, если там был 0, либо
...
big_decimal big_decimal_set_bit(big_decimal in, int index) {
  big_decimal bit = big_decimal_get_bit(index);
  return big_decimal_not(
      big_decimal_and(big_decimal_not(in), big_decimal_not(bit)));
}
/**
...
/**
 * @brief Функция возвращает дополнительный код мантиссы полученного
 * big_decimal. Знак превдварительно зануляется, чтобы не перевернулся.
...
big_decimal remove_trail_zero(big_decimal in) {
  ...
  if (exp > 0) {
    ...
    big_decimal result = big_decimal_get_zero();
    big_decimal_div_big_int(in, decimal_to_big_decimal(s21_decimal_get_ten()),
                            &result, &remainder);
