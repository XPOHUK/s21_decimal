...
int big_decimal_round_to_decimal(big_decimal in, s21_decimal* res) {
  ...
  // При умножении кспонента может оказаться выше 28. Соответственно надо
  // пробовать понизить
  if (big_decimal_get_exp(in) > 28) {
    ...
    if (code == S21_ARITHMETIC_OK && !big_decimal_is_zero(remainder)) {
      ...
      if (code == S21_ARITHMETIC_OK) {
        // Для вычисления округляемой цифры прид`тся домножить первый результат
        // на 10 и вычесть 'то из второго результата
        big_decimal shifted_one = big_decimal_shift_left(temp_res1, 1);
        big_decimal shifted_three = big_decimal_shift_left(temp_res1, 3);
        big_decimal multied =
            big_decimal_add_big_int(shifted_one, shifted_three);
        big_decimal digit = big_decimal_add_big_int(
            temp_res2, big_decimal_set_sign(multied, 1));
        if (digit.parts[0] == 5 && (!big_decimal_is_zero(remainder) ||
                                    (big_decimal_is_zero(remainder) &&
                                     big_decimal_is_set_bit(temp_res1, 0)))) {
  ...
  if (bits_to_round > 0 && code == S21_ARITHMETIC_OK) {
    // Опытным путём получено, что если поделить это количество на 3 и отбросить
    // остаток, то получим верхний уровень округления в десятичных знаках. Но
    // сначала можно проверить нижний -- возможно впишется в разрядную сетку.
    ...
    if (big_decimal_get_exp(in) < ten_exp) {
      // Если экспонента числа меньше, чем надо округлить, то имеем очень
      // большое число, результат 1
      ...
      // Если после деления экспонента числа осталась более 28, то имеем очень
      // малое число, результат 2
      code = S21_ARITHMETIC_SMALL;
    } else {
      ...
      if (first.parts[0] > 5 ||
          (first.parts[0] == 5 && (!big_decimal_is_zero(tmp_remainder) ||
