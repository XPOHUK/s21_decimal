...
// TODO(Rambtono): Все умножения на 10 переделать на использование умножения
 ...
 * @return
 */
int big_decimal_div(big_decimal dividend, big_decimal divisor,
                    big_decimal *result) {
  s21_arithmetic_result code = S21_ARITHMETIC_OK;
  // Сначала получаем целую часть (экспонента позже может её "подвинуть")
  // Знак результата уже учтён при этом вычислении, поэтому дальнейшие надо
  // выполнять без знака
  *result = big_decimal_get_zero();
  ...
  while (!big_decimal_is_zero(remainder)) {
    ...
    if (temp_exp != 28) {
      ...
      if (big_decimal_is_zero(temp_res) ||
          big_decimal_get_not_zero_bit(temp_res) < 96) {
        temp_res = big_decimal_add_big_int(temp_res, div_res);
        // При этом сложении могло произойти переполнение decimal
        if (!big_decimal_is_zero(temp_res) &&
            big_decimal_get_not_zero_bit(temp_res) > 95) {
          over_in_sum = 1;
          break;
        }
  ...
  if (big_decimal_is_zero(*result) && big_decimal_is_zero(div_res) &&
    ...
    if (div_res.parts[0] > 5 ||
        (div_res.parts[0] == 5 && (!big_decimal_is_zero(remainder) ||
