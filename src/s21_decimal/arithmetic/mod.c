  ...
  if (!s21_is_correct_decimal(value_1) || !s21_is_correct_decimal(value_2) ||
    ...
    *result = value_1;
    ...
    if (code == S21_ARITHMETIC_OK) {
      int temp_exp =
          big_decimal_get_exp(dividend) - big_decimal_get_exp(divisor);
      while (temp_exp > 0) {
        big_decimal tmp_rem = big_decimal_get_zero();
        big_decimal_div_big_int(
            res, decimal_to_big_decimal(s21_decimal_get_ten()), &res, &tmp_rem);
        temp_exp--;
      }
      while (temp_exp < 0) {
        big_decimal_mul(res, decimal_to_big_decimal(s21_decimal_get_ten()),
                        &res);
        big_decimal_mul(remainder,
